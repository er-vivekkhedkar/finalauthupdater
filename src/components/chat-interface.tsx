...
export function ChatInterface() {
  ...
  const renderMessage = (message: Message) => (
    ...
    >
      <div className={cn(
        "flex items-start space-x-3 w-full sm:max-w-[85%] md:max-w-[75%] group",
          ...
          >
            ...
            >
              {copiedMessageId === message.id ? (
                ...
                <span className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-600">
                  <Copy className="w-3.5 h-3.5" />
                </span>
              )}
            </button>
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] sm:h-[calc(100vh-8rem)] md:h-[calc(100vh-12rem)] w-full max-w-7xl mx-auto bg-gradient-to-b from-white to-primary-50/50 backdrop-blur-xl shadow-2xl rounded-2xl overflow-hidden border border-primary-100/50">
      <div className="flex-1 flex flex-col h-full relative w-full">
        <div className={cn(
          "flex-1 overflow-y-auto scroll-smooth scrollbar-thin scrollbar-thumb-primary-200 scrollbar-track-transparent p-2 sm:p-4 md:p-6",
          isInitialView ? "flex flex-col items-center justify-center" : "space-y-4 sm:space-y-6"
        )}>
          {isInitialView && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              ...
              <motion.div
                ...
                className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 w-full max-w-2xl mx-auto mt-4 px-4 pb-4 sm:pb-6"
              >
                {suggestions.map((item, index) => (
                  <motion.div
                    ...
                    onHoverStart={() => setHoveredSuggestion(item.title)}
                  ...
                  >
                    <motion.button
                      ...
                      className={cn(
                        "w-full flex items-start p-4 rounded-xl shadow-md transition-all border border-primary-100/50",
                        "bg-white/95 hover:shadow-xl hover:bg-white duration-200"
                      )}
                    >
                      <div className={cn(
                        ...
                        <motion.div
                          animate={{ rotate: hoveredSuggestion === item.title ? 360 : 0 }}
                      ...
                      <div className="ml-3">
                        <div className="font-medium text-primary-700">{item.title}</div>
                    ...
                    <AnimatePresence>
                      {hoveredSuggestion === item.title && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: 10 }}
                          className="absolute top-full left-0 right-0 mt-2 p-3 bg-white/95 rounded-xl shadow-xl border border-primary-100/50 z-10 backdrop-blur-sm"
                        >
          ...
          {isThinking && (
            ...
            >
              <div className="flex items-start space-x-3 max-w-[75%]">
                ...
                <div className="flex space-x-2 items-center bg-white rounded-2xl py-3 px-4 shadow-sm border border-gray-100">
                  <motion.span 
              ...
              <div className="flex gap-2 max-w-5xl mx-auto">
                <div className="flex-1 relative">
                  <Input
                    ...
                    onChange={(e) => setInput(e.target.value)}
                    placeholder={placeholders[currentPlaceholder]}
                    className="h-10 sm:h-12 px-3 sm:px-4 pr-[45px] sm:pr-[100px] rounded-xl text-sm sm:text-base bg-white/95 shadow-md border-primary-200/50 transition-all focus:ring-2 focus:ring-primary-500/50"
                    disabled={isLoading}
                  />
                  <Button
                    type="submit"
                    disabled={isLoading || !input.trim()}
                    className="absolute right-1 sm:right-2 top-1/2 -translate-y-1/2 h-8 sm:h-10 w-8 sm:w-10 p-0 bg-primary-600 hover:bg-primary-700"
                  >
                    {isLoading ? (
